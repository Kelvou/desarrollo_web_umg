...
router.post('/registro/:dpi', async (req, res) => {
    try {
        ...
        if (existingUser) {
            return res.status(400).json({ error: 'Ya existe un usuario con el mismo DPI, NIT o Correo Electrónico' });
        }
        if (!dpi || !nombres || !apellidos || !fechaNacimiento || !direccionEntrega || !nit || !numeroTelefonico || !correoElectronico || !clave || !validacionClave) {
            ...
            if (req.body.nit !== undefined || req.body.correoElectronico !== undefined) {
                ...
                if (existingUser) {
                    if (existingUser.dpi !== req.dpi && existingUser.nit === req.body.nit) {
                        return res.status(400).json({ error: 'No se puede duplicar el NIT' });
                    }
                    if (existingUser.dpi !== req.dpi && existingUser.correoElectronico === req.body.correoElectronico) {
                        return res.status(400).json({ error: 'No se puede duplicar el Correo Electrónico' });
                    }
                }
            }
...
router.delete('/perfil/:dpi', verifyToken, async (req, res) => {
    ...
    if (dpiBuscar == req.dpi) {
        try {
            ...
            if (usuarioEliminado) {
                res.json({ mensaje: 'Usuario eliminado con éxito', "correoElectronico": req.correoElectronico });
            } else {
                res.status(404).json({ error: 'Usuario no encontrado' });
            }
        } catch (error) {
